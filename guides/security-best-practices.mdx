...
description: Comprehensive security checklist covering secrets management, input validation, SQL injection, XSS, CSRF, authentication, and more
icon: shield-halved
---
...
<CardGroup cols={2}>
  <Card title="Authentication & Authorization" icon="user-lock">
    Implementing login, user roles, permissions
  </Card>
  
  <Card title="User Input & File Uploads" icon="file-arrow-up">
  ...
  <Card title="Sensitive Data" icon="key">
...
<Steps>
  ...
  <Step title="Validate at Startup">
    ```typescript
...
```typescript ✅ SAFE - Parameterized Query
// Supabase
...
<CodeGroup>
```typescript ❌ WRONG - localStorage (XSS vulnerable)
localStorage.setItem('token', token)
```

```typescript ✅ CORRECT - httpOnly cookies
...
<CodeGroup>
```typescript ❌ WRONG - Logging sensitive data
console.log('User login:', { email, password })
console.log('Payment:', { cardNumber, cvv })
```

```typescript ✅ CORRECT - Redact sensitive data
...
<CodeGroup>
```typescript ❌ WRONG - Exposing internal details
catch (error) {
  return NextResponse.json(
...
```typescript ✅ CORRECT - Generic error messages
catch (error) {
...
<Steps>
  ...
  <Step title="Review Entire Codebase">
...
<CardGroup cols={2}>
  <Card title="security-reviewer Agent" icon="user-secret">
    Specialized agent for vulnerability detection and security audits.
  </Card>
  
  <Card title="/security-scan Command" icon="shield">
    Run AgentShield security auditor (1282 tests, 102 rules).
  </Card>
  
  <Card title="security-review Skill" icon="clipboard-check">
    Comprehensive security checklist and patterns.
  </Card>
  
  <Card title="AgentShield" icon="robot">
    External tool: `npx ecc-agentshield scan --fix`
  </Card>
</CardGroup>
